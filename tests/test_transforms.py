...
def test_match_status_occurrence_garbage_falls_back_to_active_scheduled():
    # Garbage occurrence_datetime -> fall back to status==active -> scheduled.
    result = extract_market_fields(
...
def test_match_status_occurrence_empty_falls_back_to_active_scheduled():
    # No occurrence_datetime at all -> status==active -> scheduled.
    result = extract_market_fields(
        _per_match_market(),
    )
    assert result["match_status"] == "scheduled"
